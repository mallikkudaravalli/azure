...
resourcegroup_name = 'kmr'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
storageaccount_name = 'kmr'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
location = 'eastus'
###
...
cars.price = 15000
table_service.insert_entity('itemstable', cars)
print('Created entry for carolla...')
cars = Entity()
cars.PartitionKey = 'carsmenu'
cars.RowKey = '003'
cars.make = 'Tayota'
cars.model = 'Highlander'
cars.year = '2017'
cars.color = 'White'
cars.price = 38000
table_service.insert_entity('itemstable', cars)
print('Created entry for highlander...\n')
# A partition key tracks how like-minded entries in the Table are created and queried.
# A row key is a unique ID for each entity in the partition
# These two properties are used as a primary key to index the Table. This makes queries much quicker.
coffee = Entity()
...
print('Created entry for a Mocha...\n')
time.sleep(1)
coffee = Entity()
coffee.PartitionKey = 'coffeestore'
coffee.RowKey = '005'
